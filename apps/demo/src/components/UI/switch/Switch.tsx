...
export function Switch<T extends string>({ label, checked, handleChange, disabled }: SwitchProps<T>) {
  return (
    <HeadlessUISwitch.Group as="div" className="mt-2">
      <HeadlessUISwitch.Label className="p-3 align-top font-medium">{label}</HeadlessUISwitch.Label>
      <HeadlessUISwitch
        checked={checked}
        onChange={(checked: boolean) => handleChange(checked, label)}
        disabled={disabled}
        className={`${
          disabled ? 'bg-gray-200' : 'ui-checked:bg-blue-700 ui-not-checked:bg-gray-200'
